...
async function enrichJobFromDetail(job, source) {
  ...
  try {
    ...
    return {
      ...
      employer: extractEmployerSafe(combinedContext, job.employer || source.name),
      startInfo: extractStartInfoSafe(combinedContext) || job.startInfo || "",
...
function extractCandidates(html, source) {
  ...
  $("a[href]").each((_, element) => {
    ...
    candidates.push({
      ...
      employer: extractEmployerSafe(combined, source.name),
